  ...
  return (
    <Link to={`/zone/${id}`} className="block">
      <div className={cn(
        ...
        isActive ? "nav-item-active dark:bg-gray-700 dark:text-white" : "nav-item dark:text-gray-300",
        isActive && "dark:bg-gray-700"
      )}>
        <div className="flex items-center gap-2">
          <span className="text-xs text-zinc-600 dark:text-zinc-400">▶</span>
          <span className={isActive ? "dark:text-white" : "dark:text-gray-300"}>{name}</span>
        </div>
